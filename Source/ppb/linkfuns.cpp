...
#define TRACE_BUFF_LEN 448

extern bool ServoCycle;
//The lower 12 bits of each UsecVals entry is the Usec setting
//for a particular servo PWM control. The high four bits
//are flags currently only SERVO_UPDATE_FLAG == 0X8000
//to indicate that a new value has been loaded. Since loading PCA9685
//controller is somewhat slow (via I2C), values are actually loaded
//by a routine UpdateServo() called after every second msec interrupt
//It scans UsecVals and updates one value (at most) on each call.
...
//Allows sending new values for up to 16 different servos of one of the
 //groups. We have five groups of servos, each with potentially
//16 servos. The first group is controlled by Arduino pins
//Probably less than 16 will be implemented, (currently six).
//The next four groups have 16 servos each controlled by one of
//potentially four PCA9685 boards. Since we can send a variable number
//of values (up to 16) in a call, we code which servo of the group a
//value is for in the upper four bits of the word and the Usec setting
...
//Load values into update array (if PCA9685 controlled servo) so
//they can be loaded gradually by UpdateServo() called every other msec.
//in background. Load process is slow via I2C and we do not want to
//hold up link calls for actual loading.
void LoadServoUsec(U8 ServoIndex, U16 Usec)
{
    if(ServoIndex <= 15)
    {
        SetServoUsec(ServoIndex, Usec);
        return;
    }
    if(ServoIndex < NUM_SERVO_GROUPS * SERVO_GP_SIZE)
    {
        UsecVals[ServoIndex] = Usec | SERVO_UPDATE_FLAG;
    }
}
