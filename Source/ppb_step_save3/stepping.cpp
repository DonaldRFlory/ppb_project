...
BWUnion S4Period, S4PeriodCount;
...
void InitStepping()
{
    ...
    S1Period.I = S2Period.I = S4Period.I =256;
    if(digitalRead(AUX1))
...
ISR(TIMER0_COMPA_vect)
{
    ...
    if(S1StepCount)
    {
        ...
        {
            //If fractional part is too small, we make it bigger and
            //flag the adjustment so we time only a half full count
            //for next full count timing cycle
            S1Period.B[0] += 128;
            S1AdjustFlag = true;
        }
        else
...
void Ramp3()
{
    ...
    //We assume in Ramp3() code that we will never command moves very close to 65535,
    ...
    switch(StepCtrl[SI].RampState)
    {
        ...
        case RAMP_UP:
            ...
            {
                Steps = StepCtrl[SI].Infl1 - StepCtrl[SI].TrigStep;
